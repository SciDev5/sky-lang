...
use super::{
    ast2::{
        AST2AnonymousFunction, AST2Block, AST2BlockStructInit, AST2CompoundPostfixContents,
        AST2Expression, AST2Literal, AST2OptionallyTypedIdent, AST2TypedIdent,
        AST2VarAccessExpression,
    },
    ops::SLOperator,
...
impl<'a, 'token_content> Tokens<'a, 'token_content> {
    ...
    */
            ...
            loop {
                match {
                    ...
                    Some(SLToken::Comment { documenting, .. }) => {
                        if *documenting {
                            self.diagnostics.push(ParseDiagnostic::UnexpectedDocComment);
                        }
                        continue;
                    }
                    token => return token,
    ...
    fn try_literal(&mut self) -> Option<AST2Literal> {
        self.parse_optional(|t| match t.try_next()? {
            SLToken::Float {
                value,
                imaginary: false,
            } => Some(AST2Literal::Float(*value)),
            SLToken::Float {
                value,
                imaginary: true,
            } => Some(AST2Literal::Complex(Complex64::new(0.0, *value))),
            SLToken::Int {
                value,
                imaginary: false,
            } => Some(AST2Literal::Int(*value)),
            SLToken::Int {
                value,
                imaginary: true,
            } => Some(AST2Literal::Complex(Complex64::new(0.0, *value as f64))),
            SLToken::Bool(value) => Some(AST2Literal::Bool(*value)),
            // TODO strings
            _ => None,
        })
    }
    ...
    fn parse_expr_no_infix<const ALLOW_BLOCK_POSTFIX: bool>(
        ...
        let mut expr = match parse_first!(self;
            ...
            // -- literals -- //
            try_parse_anonfunc => |func| AST2Expression::AnonymousFunction(func?),
            try_literal => |literal| AST2Expression::Literal(literal),
            ...
            // -- identifiers -- //
            try_ident => |ident| AST2Expression::Ident(ident),
        ...
        for affix in postfixes.into_iter().chain(prefixes.into_iter().rev()) {
            expr = match affix {
                Ok(ExprAffix::Op(op)) => AST2Expression::UnaryOp {
                    op,
                    value: Box::new(expr),
                },
                Ok(ExprAffix::Compound(contents)) => AST2Expression::CompoundPostfix {
                    target: Box::new(expr),
                    contents,
                },
                Err(recoverable) => {
        ...
        if let Some(op) = self.parse_optional(|t| {
            ...
            Some(Ok(ExprAffix::Compound(
                if self.try_bracket_open(BracketType::Curly).is_some() {
                    ...
                    AST2CompoundPostfixContents::BlockStructInit(contents)
                    ...
                    AST2CompoundPostfixContents::PropertyAccess(ident)
                } else {
            ...
            Some(Ok(ExprAffix::Compound(match bracket_type {
                BracketType::Square => {
                    ...
                    AST2CompoundPostfixContents::Index(indices)
                }
    ...
    ///
    /// `target.{ <call here> ... }`
    fn finish_parse_block_struct_init(&mut self) -> ParseResult<AST2BlockStructInit> {
        ...
        while let Some(v) = self.parse_optional_result(
            ...
            |t, (doc_comment, ident)| {
                let expr = if t.try_separator(SeparatorType::Colon).is_some() {
                    ...
                    AST2Expression::Ident(ident.clone())
                };
                Ok((doc_comment, ident, expr))
            },
        ) {
            let (doc_comment, ident, expr) = match v {
                Ok(v) => v,
                Err(recoverable) => {
                    if recoverable {
                        continue;
            ...
            properties.push((ident, doc_comment, expr));
        }
        Ok(AST2BlockStructInit { properties })
    }

    fn try_parse_expr_parentheses(&mut self) -> Option<ParseResult<AST2Expression>> {
    ...
    fn try_parse_anonfunc(&mut self) -> Option<ParseResult<AST2AnonymousFunction>> {
    ...
    fn finish_parse_anonfunc(&mut self) -> ParseResult<AST2AnonymousFunction> {
        ...
        Ok(AST2AnonymousFunction { params, block })
    }
    ...
    fn parse_ident_optionally_typed(&mut self) -> ParseResult<AST2OptionallyTypedIdent> {
        // TODO destructures
        ...
        Ok(AST2OptionallyTypedIdent { ident, ty })
    }
    fn parse_ident_typed(&mut self) -> ParseResult<AST2TypedIdent> {
        // TODO destructures
        let ident = self.ident()?;
        ...
        Ok(AST2TypedIdent { ident, ty })
    }

    fn try_parse_var_declare_expr(&mut self) -> Option<ParseResult<AST2Expression>> {
        self.parse_optional_result(|t| {
            ...
            Some(doc_comment)
                ...
                let AST2OptionallyTypedIdent { ident, ty } = t.parse_ident_optionally_typed()?;
                ...
                Ok(AST2Expression::VarDeclare { doc_comment, ident, writable, initial_value, ty })
        })
    }

    fn try_parse_var_assign_expr(&mut self) -> Option<ParseResult<AST2Expression>> {
        self.parse_optional_result(|t| {
            let target = {
                // TODO advanced varaccessexpression
                let ident = t.try_ident()?;
                AST2VarAccessExpression::Var { ident }
            };
            ...
            Ok(AST2Expression::Assign { target, op, value })
        })
    }
    ...
    /// )
    /// ````
    fn try_parse_static_function_declaration(&mut self) -> Option<ParseResult<AST2Expression>> {
        self.parse_optional_result(
            |t| {
                let doc_comment = t.try_doc_comment();
            ...
            |t, doc_comment| {
                ...
                Ok(AST2Expression::FunctionDefinition {
                    doc_comment,
                    ident,
                    params,
                    return_ty,
    ...
    fn try_parse_static_struct_declaration(&mut self) -> Option<ParseResult<AST2Expression>> {
        self.parse_optional_result(
            |t| {
                let doc_comment = t.try_doc_comment();
            ...
            |t, doc_comment| {
                ...
                Ok(AST2Expression::StructDefinition {
                    doc_comment,
                    ident,
                    properties,
                })
...
enum ExprAffix {
    Op(SLOperator),
    Compound(AST2CompoundPostfixContents),
}
/*
...
*/
pub fn parse(tokens: Vec<SLToken>) -> (Result<AST2Block, ()>, Vec<ParseDiagnostic>) {
...
mod test {
    use crate::parse::{
        ast2::{AST2Expression, AST2Literal},
        ops::SLOperator,
        parser2::ParseDiagnostic,
    ...
    fn one_plus_one() {
        ...
        assert_eq!(
            parse(tokens),
            (
                Ok(vec![AST2Expression::BinaryOp {
                    op: SLOperator::Plus,
                    lhs: Box::new(AST2Expression::Literal(AST2Literal::Int(1))),
                    rhs: Box::new(AST2Expression::Literal(AST2Literal::Int(1)))
                }]),
                vec![],
            )
        );
